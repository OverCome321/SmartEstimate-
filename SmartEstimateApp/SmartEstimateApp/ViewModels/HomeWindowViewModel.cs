...
{
    public class HomeWindowViewModel : PropertyChangedBase
    {
        // Логика уведомлений и загрузки
        ...
        public string SuccessMessage { get => _successMessage; set => SetProperty(ref _successMessage, value, nameof(SuccessMessage)); }
        ...
        public HomeWindowViewModel()
        {
            ...
            _errorTimer.Tick += (s, e) => { IsErrorVisible = false; _errorTimer.Stop(); };

            _successTimer = new DispatcherTimer { Interval = TimeSpan.FromSeconds(5) };
            _successTimer.Tick += (s, e) => { IsSuccessVisible = false; _successTimer.Stop(); };

        }

        private void UpdateDateTime() => CurrentDateTime = DateTime.Now.ToString("dd.MM.yyyy HH:mm");
        public void ShowError(string message) { ErrorMessage = message; HideSuccess(); IsErrorVisible = true; if (_errorTimer.IsEnabled) _errorTimer.Stop(); _errorTimer.Start(); }
        public void HideError() { IsErrorVisible = false; if (_errorTimer.IsEnabled) _errorTimer.Stop(); }
        public void ShowSuccess(string message) { SuccessMessage = message; HideError(); IsSuccessVisible = true; if (_successTimer.IsEnabled) _successTimer.Stop(); _successTimer.Start(); }
        public void HideSuccess() { IsSuccessVisible = false; if (_successTimer.IsEnabled) _successTimer.Stop(); }
